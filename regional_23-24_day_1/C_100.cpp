...
void fillVector(vector<vector<pll>>& partSums, vector<vector<ll>>& part) {
    partSums.resize(part.size());
    for (ll mask = 0; mask < part.size(); ++mask) {
        partSums[mask].resize(part[mask].size());
        for (ll mask1 = 0; mask1 < part[mask].size(); ++mask1) {
            partSums[mask][mask1] = {part[mask][mask1], mask1};
        }
        sort(partSums[mask].begin(), partSums[mask].end());
    }
}

ll findSum(vector<vector<pll>>& partSums, ll mask, ll sum) {
   pll p = {sum, 0};
   auto getRes = lower_bound(partSums[mask].begin(), partSums[mask].end(), p);
   if (getRes != partSums[mask].end() && (*getRes).f == sum) {
        return (*getRes).s;
   }
   return -1;
}
...
void printPart(vector<vector<ll>>& part) {
    for (ll mask = 0; mask < part.size(); ++mask) {
        for (ll mask1 = 0; mask1 < part[mask].size(); ++mask1) {
            cout << mask << " " << mask1 << " " << part[mask][mask1] << endl;
        }
    }
}
